...
class BinaryTree {
    ...
    while (true) {
      if (item < current.item) {
        ...
        if (current.left === null) {
          current.left = this.createChild(item, current);
        ...
        if (current.right === null) {
          current.right = this.createChild(item, current);
            this.maintain(current);
          return;
        }
        current = current.right;
      } else {
        // Item already exists in the tree; do not add duplicates
        return;
      }
    }
  }
  ...
  dfs(node) {
    ...
    this.dfs(node.left);    // Traverse the left subtree
    console.log(node.item); // Process the current node
    this.dfs(node.right);   // Traverse the right subtree
  }
